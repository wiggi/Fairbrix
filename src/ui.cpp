...
void CSendDialog::OnButtonSend(wxCommandEvent& event)
{
    ...
    TRY_CRITICAL_BLOCK(cs_sendlock)
    {
        ...
        {
            wxMessageBox(_("Error in amount  "), _("Send FBX"));
            return;
        }
        if (nValue > pwalletMain->GetBalance())
        {
            wxMessageBox(_("Amount exceeds your balance  "), _("Send FBX"));
            return;
        }
        if (nValue + nTransactionFee > pwalletMain->GetBalance())
        {
            wxMessageBox(string(_("Total exceeds your balance when the ")) + FormatMoney(nTransactionFee) + _(" transaction fee is included  "), _("Send FBX"));
            return;
        }
        ...
        else
        {
            ...
            if (!addr.IsValid())
            {
                wxMessageBox(_("Invalid address  "), _("Send FBX"));
                return;
            }
...
wxMenu* CMyTaskBarIcon::CreatePopupMenu()
{
    ...
    pmenu->Append(ID_TASKBAR_RESTORE, _("&Open Fairbrix"));
    pmenu->Append(ID_TASKBAR_SEND, _("&Send FBX"));
